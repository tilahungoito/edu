...
import { useAuthStore, mapBackendUserToFrontendUser } from '@/app/lib/store/auth-store';
...
export default function ProfilePage() {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const mappedUser = mapBackendUserToFrontendUser(updatedUser);
            setUser(mappedUser);
            setNotification({ open: true, message: 'Profile image updated successfully!', severity: 'success' });
        } catch (error) {
            console.error('Error uploading profile image:', error);
            setNotification({ open: true, message: 'Failed to upload profile image.', severity: 'error' });
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <Grid container spacing={4} alignItems="center">
                        <Grid size="auto">
                            <Box sx={{ position: 'relative' }}>
                                ...
                                />
                                <Avatar
                                    src={user?.profilePicture ? `${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') || 'http://localhost:7000'}${user.profilePicture}` : undefined}
                                    sx={{
                                        width: 120,
                                        height: 120,
