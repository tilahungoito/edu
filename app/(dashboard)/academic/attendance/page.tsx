...
export default function AttendancePage() {
    ...
    const markBulkAttendanceMutation = useMutation({
        mutationFn: async (vars: { records: any[] }) => {
            return attendanceService.markBulkStudent(vars);
        }
    });
    ...
    const handleSubmitAll = async () => {
        ...
        const records = enrollments.map(enrollment => ({
            ...
            status: attendanceData[enrollment.id] || 'PRESENT',
            date: selectedDate,
            remarks: remarks[enrollment.id]
        }));

        try {
            await markBulkAttendanceMutation.mutateAsync({ records });
                                    ...
                                    {selectedCourse ? (
                                        <Box>
                                            ...
                                            <Typography variant="caption" color="text.secondary">
                                                Code: {selectedCourse.code} | Credits: {selectedCourse.credit}
                                            </Typography>
                                        </Box>
                                    ) : (
                            ...
                            <Box sx={{
                                ...
                                <Box sx={{ display: 'flex', gap: 1 }}>
                                    ...
                                    <Button
                                        ...
                                        disabled={!enrollments?.length || markBulkAttendanceMutation.isPending}
                                        sx={{ borderRadius: 2, fontWeight: 700 }}
                                    >
                                        {markBulkAttendanceMutation.isPending ? 'Saving...' : 'Submit All'}
                                    </Button>
                                </Box>
                            </Box>
