...
export function UserDialog({ open, user: editingUser, onClose, onSuccess }: UserDialogProps) {
    ...
    useEffect(() => {
        ...
        // Auto-set scopeId if currentUser has a scope and is creating at the same level (e.g. Institution Admin)
        const targetRole = formData.targetRole; // Use formData.targetRole for this check
        if (!targetRole || !user) return;

        const schoolRoles = ['INSTRUCTOR', 'STUDENT', 'REGISTRAR', 'ACCOUNTANT'];
        if (schoolRoles.includes(targetRole) && user.tenantType === 'school') {
            setFormData(prev => ({ ...prev, scopeId: user.tenantId }));
        }
    }, [open, availableRoles, formData.targetRole, user]);
